...
export default class StreamZip extends events.EventEmitter {
  ...
  constructor(file: File, storeEntries = true, public chunkSize?: number, skip_valid = false) {
    super();
    this.file = file;
  ...
  protected readCentralDirectory() {
    ...
    this.op = {
      win: new FileWindowBuffer(this.file),
  ...
  stream(entry: ZipEntry) {
    return this.openEntry(entry)
      .then(([_, entry]) => {
        ...
        var entryStream = new EntryDataReaderStream(this.file, offset, entry.compressedSize);
  ...
  async openEntry(entry: ZipEntry | string) {
    ...
    await new FsRead(this.file, buffer, 0, buffer.length, entry.offset).read();
  ...
  async entryData(entry: ZipEntry | string) {
    ...
    await new FsRead(this.file, data, 0, entry.compressedSize, this.dataOffset(entry)).read();
...
export async function createTrimmedArchive(file: File) {
  const s = new StreamZip(file, true);

  const builded = new JSZip();

  await new Promise((resolve, reject) => {
    s.on('ready', async () => {
      console.log('Entries read: ' + s.entriesCount);
      console.log(Object.values(s.entries))

      for (const entry of Object.values(s.entries)) {
          if (entry.name.startsWith('tweet_media')) {
            continue;
          }

          if (entry.isDirectory) {
            builded.folder(entry.name);
          }
          else {
            try {
...
class FsRead {
  ...
  async read() {
    ...
    const readed = await readFromFile(
      this.file, 
      this.buffer, 
...
class FileWindowBuffer {
  ...
  async read(pos: number, length: number) {
    ...
    this.position = pos;
    this.fsOp = new FsRead(this.file, this.buffer, 0, length, this.position);
  ...
  async expandRight(length: number) {
    ...
    this.fsOp = new FsRead(this.file, this.buffer, offset, length, this.position + offset);
  ...
  async moveRight(_: number, callback: Function, shift: number) {
    ...
    this.position += shift;
    this.fsOp = new FsRead(this.file, this.buffer, this.buffer.length - shift, shift, this.position + this.buffer.length - shift);
...
class EntryDataReaderStream extends stream.Readable {
  ...
  constructor(public file: File, public offset: number, public length: number) {
    super();
  }
  ...
  _read(n: number) {
    ...
    if (buffer.length) {
      readFromFile(this.file, buffer, 0, buffer.length, this.offset + this.pos).then(read => this.readCallback(read, buffer));
    } else {
      this.push(null);
    }
  }
...
class EntryVerifyStream extends stream.Transform {
  ...
  constructor(baseStm: EntryDataReaderStream, crc: number, size: number) {
    super();
